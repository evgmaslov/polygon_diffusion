...
def corrupt(polygons, connections_graph, corrupt_sequences):
...
def main(rplan_path, path_to_save):
    names = os.listdir(rplan_path)
    dataset_path = os.path.join(path_to_save, "rplan_json")
    os.mkdir(dataset_path)
    failed_names = []
    for name in tqdm(names):
        path = os.path.join(rplan_path, name)
        img = np.asarray(Image.open(path))
        try:
            json_dict = rplan_img_to_dict(img)
            if json_dict == None:
                failed_names.append(name)
                continue
            flat_name = name.split(".")[0]
            new_name = f"{flat_name}.json"
            new_path = os.path.join(dataset_path, new_name)
            with open(new_path, "w+") as f:
                json.dump(json_dict, f)
        except:
            failed_names.append(name)
    with open("failed_names.json", "w+") as f:
        json.dump(failed_names, f)

if __name__ == "__main__":
    rplan_path = "/home/evgmaslov/Files/rplan_image/dataset/floorplan_dataset"
