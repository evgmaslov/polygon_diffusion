...
def main(rplan_dict_path, path_to_save):
    names = os.listdir(rplan_dict_path)
    dataset_path = os.path.join(path_to_save, "rplan_json_corrupt")
    if not os.path.exists(dataset_path):
        os.mkdir(dataset_path)
    existed_files = os.listdir(dataset_path)

    max_n_rooms = 8
    corrupt_sequences = {i: get_corrupt_sequences(i, max_n_seq=100000) for i in range(2, max_n_rooms + 1)}
    
    failed_names = []
    for name in tqdm(names):
        if name in existed_files:
            continue
        path = os.path.join(rplan_dict_path, name)
        ...
        n_rooms = len(rooms)
        if n_rooms > max_n_rooms:
            continue
        
        try:
            polygons = [room["polygon"] for room in rooms]
            polygons = [[np.expand_dims(np.array(v), axis=1) for v in np.array(polygon).T.tolist()] for polygon in polygons]
            corrupted_polygons, cor_seq = corrupt(polygons, graph, corrupt_sequences[n_rooms], return_seq=True)
            for i in range(n_rooms):
                json_dict["rooms"][i]["cor_polygon"] = np.concatenate(corrupted_polygons[i], axis=1).tolist()
            json_dict["cor_sequence"] = cor_seq
            ...
            mode = "w+" if not os.path.exists("failed_names_corrupt.json") else "w"
            with open("failed_names_corrupt.json", mode) as f:
                json.dump(failed_names, f)

if __name__ == "__main__":
    rplan_path = "/home/evgmaslov/Files/rplan_json_base"
    path_to_save = "/home/evgmaslov/Files"
    main(rplan_path, path_to_save)